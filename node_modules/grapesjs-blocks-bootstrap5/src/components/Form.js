...
export default (dc, traits, config = {}) => {
  ...
  dc.addType('form', {
    model: defaultModel.extend({
      ...
      updateFormState() {
        const state = this.get('formState');
        switch (state) {
          case 'success':
      ...
      showState(state) {
        ...
        const successModel = this.getStateModel('success');
        const failModel = this.getStateModel('error');
        const successStyle = successModel.getStyle();
        const failStyle = failModel.getStyle();
      ...
      getStateModel(state) {
        ...
        const stateName = `form-state-${st}`;
        let stateModel;
        const comps = this.get('components');
        for (let i = 0; i < comps.length; i++) {
          const model = comps.models[i];
